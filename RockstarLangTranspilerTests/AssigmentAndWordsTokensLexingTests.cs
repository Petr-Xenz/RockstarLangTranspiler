...
namespace RockstarLangTranspilerTests
{
    ...
    public class AssigmentAndWordsTokensLexingTests
    {
        ...
        {
            var file = "Foo is 5";
            var lexer = new Lexer(file, new ITokenFactory<Token>[] { new IsTokenFactory(), new NumberTokenFactory(), new WordTokenFactory() });
            var tokens = lexer.Lex().ToArray();

            Assert.AreEqual(5, tokens.Length);
            Assert.IsTrue(tokens[0] is WordToken n && n.Value == "Foo");
            Assert.IsTrue(tokens[1] is IsToken a && a.Value == "is");
        ...
        public void MultiWordAssigmentTest()
        {
            ...
            var lexer = new Lexer(file, new ITokenFactory<Token>[] { new AssigmentTokenFactory(), new IsTokenFactory(), new NumberTokenFactory(), new WordTokenFactory() });
            var tokens = lexer.Lex().ToArray();
            ...
            Assert.IsTrue(tokens[2] is IsToken t2 && t2.Value == "is");
